...
@property (nonatomic, assign) PDFStoreDownloadStatus status;
...
- (NSString *)downloadDirectory {
    NSString *path=[[NSString alloc]initWithFormat:@"Documents/"];
    path = [NSHomeDirectory() stringByAppendingPathComponent:path];
    NSString *downloadDirectory=path; //= [[PDFStoreManager storagePath] stringByAppendingPathComponent:@"downloads"];
    return downloadDirectory;
}

- (PDFManager *)magazine {
    if (!_magazine) {
        self.magazine = [[PDFManager alloc] init];
        _magazine.downloading = YES;
    }
    return _magazine;
}

- (void)setStatus:(PDFStoreDownloadStatus)aStatus {
    _status = aStatus;

    // remove progress view, animated
    if (progressView_ && (_status == PDFStoreDownloadStatusFinished || _status == PDFStoreDownloadStatusFailed)) {
...
- (void)startDownload {
   ...
   // PSCLog(@"downloading pdf from %@ to %@", self.URL, destPath);
    ...
    [pdfRequest setCompletionBlockWithSuccess:^(AFHTTPRequestOperation *operation, id responseObject) {
      //  PSCLog(@"Download finished: %@", self.URL);
        ...
        if (self.isCancelled) {
            self.status = PDFStoreDownloadStatusFailed;
     ...
     //   self.magazine.fileURL = destinationURL;
        self.status = PDFStoreDownloadStatusFinished;
  ...
  //  [[PDFStoreManager sharedStoreManager] addMagazinesToStore:@[self.magazine]];
}
