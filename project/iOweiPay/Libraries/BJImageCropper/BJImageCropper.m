...
- (void)setImage:(UIImage *)image {
    ...
    // imageScale = screen size / image size;
    imageScale = 1;
    
    CGFloat xscale = self.frame.size.width / image.size.width;
    CGFloat yscale = self.frame.size.height / image.size.height;
...
- (void)updateBounds {
    ...
    topLeftIndicator.center = cropView.frame.origin;
    topRightIndicator.center = rightView.frame.origin;
    bottomLeftIndicator.center = bottomView.frame.origin;
    bottomRightIndicator.center = bottomRightView.frame.origin;
...
- (void)setUnscaledCrop:(CGRect)unscaledCrop
{
    CGRect crop = CGRectMake(unscaledCrop.origin.x / imageScale, unscaledCrop.origin.y / imageScale, unscaledCrop.size.width / imageScale, unscaledCrop.size.height / imageScale);
    [self setCrop:crop];
}
...
- (UIView*)newIndicatorView
{
    ...
    view.backgroundColor = [UIColor colorWithRed:1 green:.2 blue:.2 alpha:1];
...
- (void)setup {
    ...
    topLeftIndicator = [self newIndicatorView];
    topRightIndicator = [self newIndicatorView];
    bottomLeftIndicator = [self newIndicatorView];
    bottomRightIndicator = [self newIndicatorView];
